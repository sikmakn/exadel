...
function NewsModel() {
  ...
  this.setArticles = function (articlesObjects) {
    articles = articlesObjects.map(item => new Article(
        item._id,
        item.title,
        item.summary,
        new Date(item.createdAt),
  ...
  function getArticlesByTeg(sortedArticles, filterConfig) {
    if (!(filterConfig.teg && filterConfig.teg[0].length)) { return sortedArticles; }
    if (typeof filterConfig.teg === 'string') {
      filterConfig.teg = [filterConfig.teg];
    }
    if (!Array.isArray(filterConfig.teg)) return sortedArticles;
    for (let j = 0; j < filterConfig.teg.length; j++) {
      sortedArticles = sortedArticles.filter((obj) => {
  ...
  function getArticlesByAuthor(sortedArticles, filterConfig) {
    ...
    sortedArticles = articles.filter(
      obj => obj.author.toLowerCase() === filterConfig.author.toLowerCase(),
    );
    return sortedArticles;
  }
  ...
  function getArticlesByData(sortedArticles, filterConfig) {
    if (
      filterConfig.dateBegin !== 0 && !(filterConfig.dateBegin instanceof Date)
    ) {
      return sortedArticles;
    }
  ...
  this.editArticle = function (_id, article) {
    if (!_id || typeof _id !== 'string' || !article) return false;
  ...
  this.replaceAllTegs = function (_id, tegs) {
    ...
    if (index < 0) return false;
    articles[index].teg = [];
    db.articles.update({ _id }, { teg: tegs });
